...
public class InitILrt : MonoBehaviour
{
    ...
    /// <summary>
    /// Auto bind all scripts from hotupdate DLL to a GameObject
    /// 自动绑定热更DLL中的脚本到一个GameObject
    /// </summary>
    public static void BindAllScripts()
    {
        foreach (var cb in FindObjectsOfType<ClassBind>())
        {
            ...
            {
                //添加脚本
                IType type = appDomain.LoadedTypes[$"{_class.Namespace}.{_class.Class}"];
                    ...
                    foreach (var field in _class.Fields)
                    {
                        ...
                        if (_class.BoundData)
                        {
                            if (t.GetFields().Contains(t.GetField(field.fieldName)))
                            {
                                ...
                                {
                                    Log.PrintError($"{_class.Namespace}.{_class.Class}.{field.fieldName}赋值出错：{e.Message}");
                                }
                            }
                            else
                            {
